...
namespace Provider
{
    public class EFMembership : MembershipProvider
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataManager context = new DataManager("DatabaseEntities"))
            {
                totalRecords = context.Users.Where(MatchApplication()).Distinct().Count();
                if (totalRecords <= 0)
                {
                    return users;
                }

                IEnumerable<User> userEntities = context.Users.Where(MatchApplication()).OrderBy(u => u.Username).Skip(pageIndex * pageSize).Take(pageSize);
        ...
        public override int GetNumberOfUsersOnline()
        {
            ...
            using (DataManager context = new DataManager("DatabaseEntities"))
            {
                return context.Users.Where(MatchApplication()).Where(u => u.LastActivityDate > compareTime).Distinct().Count();
            }
        }
        ...
        public override MembershipUserCollection FindUsersByName(string usernameToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataManager context = new DataManager("DatabaseEntities"))
            {
                IOrderedQueryable<User> users = context.Users.Where(MatchApplication()).Where(u => u.Username.Contains(usernameToMatch)).OrderBy(u => u.Username);
        ...
        public override MembershipUserCollection FindUsersByEmail(string emailToMatch, int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            using (DataManager context = new DataManager("DatabaseEntities"))
            {
                IOrderedQueryable<User> users = context.Users.Where(MatchApplication()).Where(u => u.Email.Contains(emailToMatch)).OrderBy(u => u.Username);
                totalRecords = users.Count();
                if (users.Count() > 0)
                {
                    foreach (User user in users.Skip(pageIndex * pageSize).Take(pageSize))
        ...
        {
            User user = context.Users.FirstOrDefault(u => u.Username == username && u.Application.Name == applicationName);
            if (user == null)
            {
                return false;
            }
        ...
        {
            if (context.Users.Where(query).Count() > 0)
            {            
            User user = context.Users.Where(query).Where(MatchApplication()).FirstOrDefault();
            if (user == null)
            {
